...
var RhinoSpecReporter = function() {
    ...
    return {
        ...
        reportRunnerResults: function(runner) {
            if (EnvJasmine.incrementalOutput) {
            	if (typeof(runner_queue_length) === "undefined") {
            		runner_queue_length = 0;
            	}
            	
            	if (runner.queue.blocks.length == runner_queue_length) {
            		// there were no tests in the latest suite
                	print([EnvJasmine.green("No tests found")].join(' '));
                	return
            	}
            	runner_queue_length = runner.queue.blocks.length;
            	// Take only the reports from the most recent suite
                var results = runner.queue.blocks[runner.queue.blocks.length-1].results();
                
                print();
                print([
                    EnvJasmine.green("Passed: " + (results.passedCount)),
                    EnvJasmine.red("Failed: " + (results.failedCount)),
                    EnvJasmine.plain("Total: " + (results.totalCount))
                ].join(' '));
            }
        },
